module type Map = sig
  type ('k, 'v) t
  (** [t] is the type of maps that bind keys of type ['k] to values of type
      ['v]. *)
  ...
  (** [of_list lst] is a map containing the same bindings as association list
      [lst]. Requires: [lst] does not contain any duplicated keys. *)
...
module AssocListMap : Map = struct
  ...
  {k1 : v1, k2 : k2, ..., kn : vn}. If a key appears more
  than once in the list, then in the map it is bound to
  the left-most occurrence in the list.
  For example, [[(k, v1); (k, v2)]] represents the map 
  {k: v1}. The empty list represents the empty map.
  RI: none *)
